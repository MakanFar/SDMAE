...
class AttentionAggregator(nn.Module):
    ...
    def forward(self, nodes, adj, ind):
        ...
        edges = np.array(adj[np.array(nodes), :].nonzero()).T
...
def run(dataset, k):
    ...
    features = nn.Embedding(num_nodes, NODE_FEAT_SIZE).to(DEVICES)
    ...
    for i in adj_lists1_1:
        for j in adj_lists1_1[i]:
            ...
            weight_dict[i][j] = np.dot(v_list1, mask)
    ...
    def func(adj_list):
        edges = [(a, b) for a in adj_list for b in adj_list[a]]
        edges = np.array(edges)
        return sp.csr_matrix((np.ones(len(edges)), (edges[:, 0], edges[:, 1])), shape=(num_nodes, num_nodes))
    ...
    enc1 = Encoder(features, NODE_FEAT_SIZE, EMBEDDING_SIZE1, adj_lists, aggs).to(DEVICES)
    ...
    optimizer = torch.optim.Adam(
        filter(lambda p: p.requires_grad, list(model.parameters()) + list(enc1.parameters()) + list(features.parameters())),
        lr=LEARNING_RATE, weight_decay=WEIGHT_DECAY
    )

    metrics = {"epoch": [], "loss": [], "accuracy": [], "f1_score": [], "auc": []}
    ...
    for epoch in range(EPOCHS + 1):
        ...
        if epoch % INTERVAL_PRINT == 0:
            ...
            for i in range(0, NUM_NODE, BATCH_SIZE):
                vals = np.arange(i, min(i + BATCH_SIZE, NUM_NODE))
                embed = model.forward(vals.tolist()).data.cpu().numpy()
                all_embedding[i:min(i + BATCH_SIZE, NUM_NODE)] = embed

            np.save(os.path.join(OUTPUT_DIR, f'embedding-{dataset}-{k}-{epoch}.npy'), all_embedding)
            pos_ratio, acc, f1_0, f1_1, f1_2, auc = logistic_embedding(k=k, dataset=dataset, epoch=epoch, dirname=OUTPUT_DIR)
            metrics["epoch"].append(epoch)
            metrics["accuracy"].append(acc)
            metrics["f1_score"].append(np.mean([f1_0, f1_1, f1_2]))
        ...
        for batch in range(NUM_NODE // BATCH_SIZE):
            ...
            loss = model.criterion(nodes, adj_lists1, adj_lists2, adj_lists1_1, adj_lists2_1, weight_dict)
    ...
    # ----------- Save metrics -----------
    metrics_path = os.path.join(OUTPUT_DIR, f"metrics-{dataset}-{k}.json")
    ...
    plt.plot(metrics["epoch"], metrics["f1_score"], label="F1 Score")
    plt.plot(metrics["epoch"], metrics["auc"], label="AUC")
    plt.xlabel("Epoch")
    plt.ylabel("Metric Value")
    plt.title(f"Training Metrics for {dataset} (k={k})")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, f"metrics-{dataset}-{k}.png"))
    plt.show()
